...
var _ = Describe("PodWatch Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind PodWatch")
			err := k8sClient.Get(ctx, typeNamespacedName, podwatch)
			if err != nil && errors.IsNotFound(err) {
				resource := &configv1alpha1.PodWatch{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance PodWatch")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			svc := service.NewWatcher(k8sClient, k8sClient.Scheme(), slack.New("BOT_TOKEN"))
			controllerReconciler := NewPodWatchReconciler(svc)
