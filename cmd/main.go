...
import (
	...
	configcontroller "github.com/mgufrone/pod-notifier/internal/controller/config"
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "459e0434.mgufrone.dev",
	...
	svc := service.NewWatcher(mgr.GetClient(), mgr.GetScheme(), slackClient)
	if err = svc.SetupIndex(mgr); err != nil {
		setupLog.Error(err, "unable to setup index")
		os.Exit(1)
	}

	if err = (configcontroller.NewPodWatchReconciler(svc)).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "PodWatch")
		os.Exit(1)
	}
	if err = (configcontroller.NewClusterPodWatchReconciler(svc)).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "ClusterPodWatch")
		os.Exit(1)
	}
